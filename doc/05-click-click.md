...
![「オブジェクト」の選択](../image/05-07-01.jpg) ![AppModelの選択](../image/05-07-02.jpg)
...
データバインドを使用している際、仮にこれまでと同じように、現在値を`"現在値:n"`のように表示するよう変更があったとしましょう。そんなときは、デザイナーから対応が可能です。つまり、目に見えるコードの範囲では一切変更がいらず、「見た目」の変更は見た目を編集することに特化した「デザイナー」だけに任せることができます。これもデータバインドのメリットの一つです。

デザイナーから現在値の表示書式を変更するには、まずプロパティ ウィンドウの「(DataBindings)」→「詳細」を選択して「...」ボタンをクリックし、「フォーマットと詳細バインド」ダイアログを表示します（図5-11）。

![データバインドの詳細](../image/05-11-01.jpg) ![「フォーマットと詳細バインド」ダイアログ](../image/05-11-02.jpg)

図5-11 「フォーマットと詳細バインド」ダイアログ

そして、「プロパティ」でTextプロパティを選択後、「形式」→「形式の種類」を「カスタム」に変更後、「カスタム書式」欄に`"現在値:0"`を入力して「OK」ボタンをクリックします（図5-12）
。

![カスタム書式設定](../image/05-12.jpg)

図5-12 カスタム書式設定

本章ではアプリのデータ、状態をどのように管理し、表示するかを学んできました。次の章では、そのデータの「妥当性」をどのように担保していくかについて学びます。
